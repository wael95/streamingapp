...
Flow:
  ...
  4. Claude sends the message via send_whatsapp (broadcast to allowlist).
...
Steps:
  ...
  2. If the list is empty, send_whatsapp("لا توجد أسهم اجتازت الفلترة اليوم.") and stop.
  3. Otherwise pick the TOP 5 using the user's cached context (holdings,
     ...
     get_fundamentals / get_technicals / get_insider_filings /
     get_deep_news on candidates before ranking.
  4. Produce a WhatsApp message in Arabic in this structure:

     أفكار شراء — <date>
     1. <TICKER> @ $<price> — <سببين مختصرين مرتبطين بسياق المستخدم>
     2. ...
     (up to 5)
     تنبيه: ليست نصيحة مالية.

  5. Call send_whatsapp with the Arabic message, then
     log_report(kind='buy', payload={picks:[...]}).

CRITICAL — OUTPUT LANGUAGE:
  The text passed to send_whatsapp MUST be in Arabic (العربية). Ticker
  symbols, prices, and percentages stay in Latin/digits. Use natural
  Modern Standard Arabic for the rest. Keep it mobile-friendly."""
